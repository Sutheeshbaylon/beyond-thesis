  ...
  return (
    <main className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard label="Drafts in progress" value={drafts} />
          ...
          <div className="bg-white border border-[#E5E5E5] rounded-lg divide-y divide-[#E5E5E5]">
            {revisionList.map((r) => {
              ...
              return (
                <Link
                  key={r.id}
                  href={`/writer/projects/${r.project_id}`}
            ...
            {correctionList.map((c) => {
              ...
              return (
                <Link
                  key={c.id}
                  href={`/writer/projects/${c.project_id}`}
                  className="flex items-center justify-between px-4 py-3 hover:bg-[#F8F8F7] transition-colors"
                >
                  <div>
      ...
      <StaffPipeline projects={projects ?? []} role="writer" />
    </main>
  )
}
